      ...
      "Transform": {
        ...
        "Variables": {
          "Size": [
            5690004131215169010,
            {
              "x": 31.959999084472656,
              "y": 11.960000038146973,
              "z": 0.0
            }
          ],
          "Rotation": [
...
FileExplorer::FileExplorer() : Window("File Explorer")
{
...
void FileExplorer::Display()
{
    ...
    if (std::filesystem::path(Editor::CurrentPath) != std::filesystem::path(Engine::GetMainPath()) && ImGui::Button("Back"))
    {
        Editor::CurrentPath = std::filesystem::canonical(Editor::CurrentPath + "/..").string() + "/";
    }
    ImVec2 Position = ImVec2(0, 30);
    int rows = 1;
    for (const auto &entry : fs::directory_iterator(Editor::CurrentPath))
    {
